// ELV route builder.
//
// For each edge in the ELV system graph, produce a polyline along the
// floor wall-perimeter that joins the central hub (VIDEO_DOOR_PHONE /
// INTERCOM / ELV_RACK) to the leaf device. Structured cabling routes
// in the CEILING zone (false ceiling / slab soffit) per low-voltage
// integrator convention.
//
// Each cable type's identity carries through to the route (cableTypeId
// + gaugeMm2 from the catalog) so quantity aggregation can sum lengths
// by cable type without re-deriving from device types.
//
// Pure: returns { routes, warnings } — no graph or state mutation.

import { getFloorWallPerimeterGraph } from '../../topology/index.js'
import { walkWallPerimeter, polylineLengthFt } from '../shared/geometry.js'
import { getZone } from '../shared/routingZones.js'
import { sizeElvBranches } from './sizing.js'

const DEFAULT_FLOOR_ID = 'F1'

// Per-edge-system route-kind labels — one per ELV sub-system.
const SYSTEM_TO_ROUTE_KIND = Object.freeze({
  CCTV:     'CCTV_CABLE',
  DATA:     'DATA_CABLE',
  SECURITY: 'SECURITY_CABLE',
  AV:       'AV_CABLE',
})
...
export function buildElvRoutes(graph, state) {
  const warnings = []
  if (!graph || !graph.edges || !state) return { routes: [], warnings }

  const sized = sizeElvBranches(graph, { state, projectSettings: state.projectSettings })
  ...
  for (const eid of edgeIds) {
    ...
    // ELV runs along CEILING (structured cabling convention).
